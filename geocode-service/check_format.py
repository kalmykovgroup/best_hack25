...
cursor.execute("""
    ...
    WHERE street LIKE '%ул.%' OR street LIKE '%пер.%'
       ...
       OR street LIKE '%б-р%' OR street LIKE '%пр-кт%'
    LIMIT 10
""")
...
for row in addr_rows:
    ...
    print(f"Адрес: {addr}")
    print(f"  - Использует запятые: {'✅' if has_commas else '❌ ПРОБЛЕМА'}")
    print(f"  - В нижнем регистре: {'❌ ПРОБЛЕМА (должен быть с заглавными)' if is_lowercase else '✅'}")
    print()
